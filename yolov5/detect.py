...
def run(
    ...
    cd = ChangeDetection(names)  # added, ChangeDetection 클래스 객체 생성
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            detected = [0 for i in range(len(names))]  # added, 현재 프레임에서 검출된 객체 상태
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    detected[int(cls)] = 1  # added, 현재 프레임에서 검출된 객체 상태
            ...
            cd.add(names, detected, save_dir, im0)  # added, ChangeDetection 클래스의 add 메서드 호출
            if view_img:
